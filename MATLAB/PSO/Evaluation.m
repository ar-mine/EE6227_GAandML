% Record
function [P_fitness, P] = Evaluation(gen, X, r, f, g, h)
    global history;
    delta = 1e-6;
    ...
    X_fitness = f + [g_norm.^2 h_norm.^2]*r;
    ...
    worst = max(f);
    best = min(f);
    fprintf(' %d     %7f     %7f     %7.4f   %.4e     %7f     %7f     %7e\n', gen, mean_, median_, SD, mean(X_fitness), best, worst, MV);
    history = [history; gen, mean_, median_, SD, mean(X_fitness), best, worst, MV];
end